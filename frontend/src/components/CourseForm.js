import { useState, useEffect } from "react";
import { useCoursesContext } from '../hooks/useCoursesContext';
import { useAuthContext } from '../hooks/useAuthContext';

const CourseForm = ({ selectedCourse, setSelectedCourse }) => {
    const { dispatch } = useCoursesContext();
    const { user } = useAuthContext();

    // Form states
    const [course_name, setCourse_name] = useState('');
    const [course_description, setCourse_description] = useState('');
    const [course_fee, setCourse_fee] = useState('');
    const [subject_area, setSubject_area] = useState('');
    const [required_result, setRequired_result] = useState('');
    const [age_limit, setAge_limit] = useState('');
    const [error, setError] = useState(null);
    const [emptyFields, setEmptyFields] = useState([]);

    // Dropdown options
    const subjectAreas = [
        "Accounting",
        "Computing",
        "Business",
        "Medicine",
        "Nursing",
        "Speech and Language",
        "Veterinarian",
        "Pharmacist",
        "Architecture",
        "Education",
        "Strategic Studies",
        "Psychology",
        "Criminology",
        "Environmental Science",
        "Environmental Management",
        "Law",
        "Media",
        "industrial design",
        "Civil Engineer",
        "Mechanical Engineer",
        "Electrical and Electronics",
        "Biotechnology",
        "Arts",
        "Music",
        "Textile and Apparel",
        "Science",
        "Social Science",
        "Aviation",
        "Astrophysics",
    ];

    // Prefill form fields when a course is selected
    useEffect(() => {
        if (selectedCourse) {
            setCourse_name(selectedCourse.course_name || '');
            setCourse_description(selectedCourse.course_description || '');
            setCourse_fee(selectedCourse.course_fee || '');
            setSubject_area(selectedCourse.subject_area || '');
            setRequired_result(selectedCourse.required_result || '');
            setAge_limit(selectedCourse.age_limit || '');
        } else {
            // Reset fields if no course is selected
            setCourse_name('');
            setCourse_description('');
            setCourse_fee('');
            setSubject_area('');
            setRequired_result('');
            setAge_limit('');
        }
    }, [selectedCourse]);

    const handleSubmit = async (e) => {
        e.preventDefault();

        if (!user) {
            setError('Please login to add or update a course');
            return;
        }

        const course = { course_name, course_description, course_fee, subject_area, required_result, age_limit };

        let response;
        if (selectedCourse) {
            // Update course if one is selected
            response = await fetch(`/api/courses/${selectedCourse._id}`, {
                method: 'PATCH',
                body: JSON.stringify(course),
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${user.token}`,
                },
            });
        } else {
            // Create a new course
            response = await fetch('/api/courses', {
                method: 'POST',
                body: JSON.stringify(course),
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${user.token}`,
                },
            });
        }

        const json = await response.json();

        if (!response.ok) {
            setError(json.error);
            setEmptyFields(json.emptyFields || []);
        } else {
            if (selectedCourse) {
                dispatch({ type: 'UPDATE_COURSE', payload: json });
                console.log('Course Updated', json);
            } else {
                dispatch({ type: 'CREATE_COURSE', payload: json });
                console.log('New Course Added', json);
            }

            // Reset form and clear selected course
            setCourse_name('');
            setCourse_description('');
            setCourse_fee('');
            setSubject_area('');
            setRequired_result('');
            setAge_limit('');
            setError(null);
            setEmptyFields([]);
            setSelectedCourse(null);
        }
    };

    return (
        <div className="bg-zinc-100 p-8 rounded-md overflow-hidden shadow-lg m-12 mx-96 hover:bg-indigo-100">
            <form onSubmit={handleSubmit}>
                <h2 className="text-center font-sans text-2xl font-semibold">
                    {selectedCourse ? 'Edit Course' : 'Add a New Course'}
                </h2>
                <div className="flex flex-wrap mx-3 mb-6">
                    <label className="block w-full px-3 mb-2 text-sm font-medium text-gray-700">Course Name:</label>
                    <input
                        className={`w-80 ml-10 h-6 text-sm ${emptyFields.includes('course_name') ? 'error' : ''}`}
                        type="text"
                        onChange={(e) => setCourse_name(e.target.value)}
                        value={course_name}
                    />

                    <label className="block w-full px-3 mb-2 text-sm font-medium text-gray-700">Description:</label>
                    <textarea
                        className={`w-80 h-12 ml-10 resize-none overflow-y-auto text-sm ${emptyFields.includes('course_description') ? 'error' : ''}`}
                        type="text"
                        onChange={(e) => setCourse_description(e.target.value)}
                        value={course_description}
                    />

                    <label className="block w-full px-3 mb-2 text-sm font-medium text-gray-700">Course Fee:</label>
                    <input
                        className={`w-80 ml-10 h-6 text-sm ${emptyFields.includes('course_fee') ? 'error' : ''}`}
                        type="text"
                        onChange={(e) => setCourse_fee(e.target.value)}
                        value={course_fee}
                    />

                    <label className="block w-full px-3 mb-2 text-sm font-medium text-gray-700">Related Subject Area:</label>
                    <select
                        className={`w-80 ml-10 h-6 text-sm ${emptyFields.includes('subject_area') ? 'error' : ''}`}
                        value={subject_area}
                        onChange={(e) => setSubject_area(e.target.value)}
                    >
                        <option value="">Select a Subject Area</option>
                        {subjectAreas.map((area) => (
                            <option key={area} value={area}>
                                {area}
                            </option>
                        ))}
                    </select>

                    <label className="block w-full px-3 mb-2 text-sm font-medium text-gray-700">Required Result:</label>
                    <input
                        className="w-80 ml-10 h-6 text-sm"
                        type="text"
                        onChange={(e) => setRequired_result(e.target.value)}
                        value={required_result}
                    />

                    <label className="block w-full px-3 mb-2 text-sm font-medium text-gray-700">Age Limit:</label>
                    <input
                        className="w-80 ml-10 h-6 text-sm"
                        type="text"
                        onChange={(e) => setAge_limit(e.target.value)}
                        value={age_limit}
                    />
                </div>
                <div className="pt-2">
                    <button className="bg-cyan-600 rounded-full hover:bg-cyan-800 justify-start py-2 px-3 text-xs font-bold cursor-pointer tracking-wider">
                        {selectedCourse ? 'Update Course' : 'Add Course'}
                    </button>
                </div>
                <div className="p-4">
                    {error && <div className="p-2 bg-red-100 rounded-md border-red-200 shadow-lg">{error}</div>}
                </div>
            </form>
        </div>
    );
};

export default CourseForm;
