...
const CourseForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if(!user) {
            setError('Please login to add a course')
            return
        }
        ...
        <div className=" bg-zinc-100 p-8 rounded-md overflow-hidden shadow-lg m-12 mx-96 hover:bg-indigo-100">
            <form onSubmit={handleSubmit}>
                ...
                <div className="flex flex-wrap mx-3 mb-6 ">
                    ...
                    <input className={`w-80 ml-10 h-6 text-sm ${emptyFields.includes('course_name') ? 'error' : ''}`}
                        type="text"
                    ...
                    <textarea className={`w-80 h-12 ml-10 resize-none overflow-y-auto text-sm ${emptyFields.includes('course_description') ? 'error' : ''}`}
                        type="text"
                    ...
                    <input className={`w-80 ml-10 h-6 text-sm ${emptyFields.includes('course_fee') ? 'error' : ''}`}
                        type="text"
                    ...
                    <input className={`w-80 ml-10 h-6 text-sm ${emptyFields.includes('subject_area') ? 'error' : ''}`}
                        type="text"
